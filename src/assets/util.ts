...
const musicGlob = import.meta.glob<string>('./music/**/*', {query: '?url', import: 'default', eager: true});
...
const audioGlob = import.meta.glob<string>('./sounds/**/*', {query: '?url', import: 'default', eager: true});
...
const fontGlob = import.meta.glob<string>('./fonts/**/*', {query: '?url', import: 'default', eager: true});
export const loadFont = async (path: string, name: string) => {
	const face = new FontFace(name, `url(${fontGlob[`./fonts/${path}`]})`, {style: 'normal', weight: '400'});
	await face.load();
